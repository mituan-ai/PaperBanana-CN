...
- 安装和运行入口更统一：新增全局 `paperbanana-pro` 主命令，保留 `paperbanana` 兼容别名，并提供 `paperbanana-pro run`、`uv sync --locked` 和提交入库的 `uv.lock`。
...
| 安装与运行 | 以仓库内脚本运行和论文复现语境为主 | 全局 `paperbanana-pro` 主命令、`paperbanana` 兼容别名、`paperbanana-pro run`、`uv sync --locked`、`uv.lock`、Evolink 接入 |
...
- 当前主 CLI 命令是 `paperbanana-pro`。
- `paperbanana` 仍然保留为兼容别名，方便本地快速启动和旧习惯过渡。
...
#### 正式支持：standalone tool install

当前正式安装合同已经从 “repo-first editable” 切到 “tool install / wheel 优先”。推荐顺序如下：

1. 本地源码直接装成全局工具：`uv tool install .`
2. 或者显式从当前目录发布入口：`uv tool install --from . paperbanana-pro`
3. 等包发布到索引后，直接执行：`uv tool install paperbanana-pro`
...
- 当前 wheel / tool install 已经不再依赖“必须从仓库根目录找到 `demo.py` / `main.py` 才能启动”。
- 如果你只是本地开发，仍然可以使用 editable 安装；但它不再是 README 的唯一正式合同。
- `uv tool install paperbanana-pro` 这条命令依赖包已经发布到可访问索引；在本地源码场景下，请优先使用 `uv tool install .` 或 `uv tool install --from . paperbanana-pro`。

#### 开发路径：本地仓库开发

如果你要在仓库里做开发、调试和测试，保留标准的锁定环境流程：
...
这里的 `uv sync --locked` 会基于 `pyproject.toml` 与已提交的 `uv.lock` 创建可复现环境。

说明：本仓库团队日常偏好复用共享 `uv tool` 环境；但作为公开 README，仍保留 `uv sync --locked` 作为标准本地开发工作流。
...
### GUI：`paperbanana-pro` / `paperbanana` / `streamlit run demo.py`
...
### CLI：`paperbanana-pro run` / `paperbanana run` / `python main.py`
...
```bash
paperbanana-pro run --task_name diagram --exp_mode demo_full --provider gemini
```
...
```bash
paperbanana-pro run --dataset_name MyDataset --task_name plot --exp_mode dev_full
```
...
├── cli.py                  # 全局 paperbanana-pro / paperbanana 命令入口
├── demo.py                 # 主 GUI
├── main.py                 # CLI 批处理入口
